...
import numpy as np
from torch.utils.data import Dataset, DataLoader
from mouth_out_filter import datafilter
...
def process_hands(input_raw):
    keys = ['mouth_71', 'mouth_72', 'mouth_73', 'mouth_74', 'mouth_75', 'mouth_76', 'mouth_77', 'mouth_78',
...
def process_skeleton(input_raw, nodes):
    ...
    input_raw = {**input_raw['face']}
    input = np.array([input_raw[jn] for jn in range(12)]).transpose(2, 1, 0)
...
def custom_collate_fn(batch):
    ...
    padded_batch_list = [
        {'face': {key: [None] * max_length for key in range(12)}} for _ in range(len(batch))]
