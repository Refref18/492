...
import numpy as np
from torch.utils.data import Dataset, DataLoader
from mouth11_filter import datafilter
...
def process_skeleton(input_raw, nodes):
    ...
    input_raw = {**input_raw['face']}
    input = np.array([input_raw[jn] for jn in range(11)]).transpose(2, 1, 0)
...
def custom_collate_fn(batch):
    ...
    padded_batch_list = [
        {'face': {key: [None] * max_length for key in range(11)}} for _ in range(len(batch))]
