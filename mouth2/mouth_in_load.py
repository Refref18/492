...
import numpy as np
from torch.utils.data import Dataset, DataLoader
from mouth_in_filter import datafilter
...
def process_hands(input_raw):
    keys = ['mouth_83', 'mouth_85', 'mouth_87', 'mouth_89', 'mouth_84', 'mouth_86', 'mouth_88', 'mouth_90']
...
def process_skeleton(input_raw, nodes):
    ...
    input_raw = {**input_raw['face']}
    input = np.array([input_raw[jn] for jn in range(8)]).transpose(2, 1, 0)
...
def custom_collate_fn(batch):
    ...
    # print(batch[0][0])
    max_length = max(
        len(sample[0]['face']['mouth_83']) for sample in batch)
    ...
    padded_batch_list = [
        {'face': {key: [None] * max_length for key in range(8)}} for _ in range(len(batch))]
