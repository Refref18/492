...
import os
import logging
from mouth_in_load import VideoPoseDataset
import torch.optim.lr_scheduler as lr_scheduler
import csv
import numpy as np
import matplotlib.pyplot as plt
from mouth_in_load import custom_collate_fn
...
graph = Graph(**{"layout": "mmpose_mouth_8", "strategy": "spatial"})
...
dataloader_test = DataLoader(
    test_dataset, batch_size=4, collate_fn=custom_collate_fn)
...
for epoch in range(10):
    ...
    with torch.no_grad():
        for j, inputs in enumerate(dataloader_test):
            ...
            # print("this are predictions : ",predictions)
            # print("this are labels : ",ground_truths)
            total_samples = len(labels)
            correct_samples = torch.sum(
                torch.tensor(predictions) == labels).item()
    ...
    val_accuracy = (torch.tensor(predictions) == torch.tensor(
        ground_truths)).sum().item() / len(ground_truths)
    val_accuracy *= 100
    val_accuracy_values.append(val_accuracy)
    all_predictions.extend(predictions)
    all_ground_truths.extend(ground_truths)
    print(
        f"Epoch {epoch + 1} train loss: {epoch_loss:.3f}, validation accuracy: {val_accuracy:.3f}")
...
confusion = confusion_matrix(all_ground_truths, all_predictions)
